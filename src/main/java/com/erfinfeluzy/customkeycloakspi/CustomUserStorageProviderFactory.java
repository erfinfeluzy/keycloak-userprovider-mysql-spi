...
public class CustomUserStorageProviderFactory implements UserStorageProviderFactory<CustomUserStorageProvider> {
    ...
    public CustomUserStorageProviderFactory() {
        ...
        configMetadata = ProviderConfigurationBuilder.create()
          ...
          .property()
            ...
            .label("JDBC URL")
            .type(ProviderConfigProperty.STRING_TYPE)
            .defaultValue("jdbc:mysql://127.0.0.1:3306/inventory?useSSL=false&allowPublicKeyRetrieval=true")
    ...
    public void validateConfiguration(KeycloakSession session, RealmModel realm, ComponentModel config) throws ComponentValidationException {
       ...
       try (Connection c = DbUtil.getConnection(config)) {
        ...
        //    c.createStatement().execute("select version()");
           log.info("[I92] Connection OK !" );
       }
       catch(Exception ex) {
           ex.printStackTrace();
           log.error("[W94] Unable to validate connection: ex={}", ex.getMessage());
